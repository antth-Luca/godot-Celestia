...
func perform_use() -> void:
	...
	ITEM_HAND_ANIMATION.speed_scale = ITEM_HAND_ANIMATION.get_animation(stack_hand.item.anim_type).length / calc_use_speed
	ITEM_HAND_ANIMATION.play(stack_hand.item.anim_type)
	stack_hand.item.use(player, calc_use_speed)
